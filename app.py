...
def get_gps():
    """
    高精度GPS取得版。
    watchPositionで約12秒間GPSを取り続けて、
    一番accuracyが小さい位置情報を採用する。
    """
        ...
        new Promise((resolve) => {
            ...
            watchId = navigator.geolocation.watchPosition(
                (position) => {
                    ...
                    if (best === null || data.accuracy < best.accuracy) {
                        best = data;
                    }
                ...
                (error) => {
                    ...
                    if (error.code === 1) {
                        message = "位置情報の許可がブロックされています。ブラウザ設定から位置情報を許可してください。";
                    } else if (error.code === 2) {
                        message = "現在地を特定できませんでした。屋外や窓際で再取得してください。";
                    } else if (error.code === 3) {
                        message = "GPS取得がタイムアウトしました。もう一度押してください。";
                    }
            ...
            setTimeout(() => {
                if (best !== null) {
                    ...
                    finish({
                        error: "GPS結果が返ってきませんでした。HTTPS、位置情報の許可、スマホ利用を確認してください。"
                    });
                }
            }, 12000);
        });
        """,
        key=f"gps_precise_{datetime.now().timestamp()}"
    )
...
def show_gps_result(gps, user_id, mode):
    ...
    if isinstance(gps, dict) and gps.get("latitude") is not None and gps.get("longitude") is not None:
        ...
        if accuracy is None:
            ...
            st.success(f"実証実験に使える精度で取得しました。GPS精度：約{round(accuracy)}m")
        elif accuracy <= 50:
            st.warning(f"現在地を取得しましたが、やや誤差があります。GPS精度：約{round(accuracy)}m")
        else:
            st.error(
                f"GPS誤差が大きいです。GPS精度：約{round(accuracy)}m。"
                "スマホを屋外・窓際に移動して、もう一度取得してください。"
            )
...
def gps_status_text():
    ...
    if accuracy <= 15:
        ...
        return f"実証向き：約{round(accuracy)}m"
    elif accuracy <= 50:
...
def is_gps_good_enough():
    return st.session_state.accuracy is not None and st.session_state.accuracy <= 50
...
with gps_top2:
    if st.button("📍 高精度GPSで現在地を取得", use_container_width=True):
        if st.session_state.mode is None:
            st.warning("先に自転車モードか車モードを選択してください。")
        else:
            with st.spinner("高精度GPSを取得中です。12秒ほど待ってください..."):
                gps = get_gps()
...
st.caption("実証実験ではスマホのSafari/Chromeで開いてください。位置情報を許可し、屋外か窓際で取得すると精度が上がります。")
...
with set_col4:
    if st.button("🔄 高精度GPSを再取得", use_container_width=True):
        with st.spinner("高精度GPSを再取得中です。12秒ほど待ってください..."):
            gps = get_gps()
...
# ==================================================
    ...
    if len(near_user) > 0:
        ...
        if mode == "自転車モード":
            title = "🚗 車接近アラート"
            target = "車"
            msg = f"近くに車が接近しています。距離は約{first_user['distance_m']}メートルです。注意してください。"
        else:
            title = "🚲 自転車接近アラート"
            target = "自転車"
            msg = f"近くに自転車が接近しています。距離は約{first_user['distance_m']}メートルです。注意してください。"
...
# ==================================================
    ...
    5. 位置情報を許可する  
    6. 高精度GPSを取得する  
    7. 危険地点接近アラート、口コミ投稿、接近アラートを確認する  
